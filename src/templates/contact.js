...
function Contact({ pageContext }) {
  ...
  const [messageTel, setMessageTel] =
    useState("");
  const [messageMessage, setMessageMessage] =
    useState("");
  ...
  useEffect(() => {
    function navBarTypeLanguage() {
      if (
        ...
        setUrl(
          `${process.env.GATSBY_CONTACT_FORM_FR}`
        );
        setMetaLang("fr");
      }
      if (
        ...
        setUrl(
          `${process.env.GATSBY_CONTACT_FORM_EN}`
        );
        setMetaLang("en");
      }
    }
