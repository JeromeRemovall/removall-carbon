...
function Offers({pageContext}){
	...
	//Form
	const dataForm = dataOffers.contactPageRecrutement;
	...
	const [messageLetter, setMessageLetter] = useState("");

	const [nameFileCV, setNameFileCV] = useState("");
	const [nameFileLetter, setNameFileLetter] = useState("");

	useEffect(() => {
		function getLanguage(){
			if(window.location.href.match("/fr$") || window.location.href.match("/fr/")){
				setUrl(`${process.env.GATSBY_RECRUITMENT_FORM_FR}`);
				setMetaLang("fr");
			}else if(window.location.href.match("/en$") || window.location.href.match("/en/")){
				setUrl(`${process.env.GATSBY_RECRUITMENT_FORM_EN}`);
				setMetaLang("en");
			}
		}
		getLanguage();
		...
		.then((response) => {
			...
			if(response.status === "validation_failed"){
				...
				for (let i = 0; i < response.invalid_fields.length; i++) { 
					...
					if(response.invalid_fields[i].error_id === "-ve-linkedin"){
						setMessageLinkedin(response.invalid_fields[i].message)
					}
					...
					if(response.invalid_fields[i].error_id === "-ve-file-cv"){
						setMessageCV(response.invalid_fields[i].message)
					}

					if(response.invalid_fields[i].error_id === "-ve-file-letter"){
						setMessageLetter(response.invalid_fields[i].message)
					}
				}
			...
			<main className="offers">
				...
				<section className="offers__content-form" id={metaLang === "fr" ? "formulaire" : "form"}>
					...
					<form className="form" method="post" action={url} onSubmit={formSubmissionHandler} >
						...
						<div className="group-input">
							<input type="text" id="email" name="email" placeholder={dataForm.adresseEmail} />
							<p className="form-message__error">{messageEmail}</p>
						</div>
						<div className="group-input">
							<input type="text" id="tel" name="tel" placeholder={dataForm.telephone} />
							<p className="form-message__error">{messageTel}</p>
						</div>
						<div className="group-input">
							<input type="text" id="linkedin" name="linkedin" placeholder={dataForm.linkedin} />
							<p className="form-message__error">{messageLinkedin}</p>
						</div>
						<div className="group-input">
							{/* <label for="file-cv">{dataForm.ajouterUnCv}{nameFileCV}</label> */}
							<label for="file-cv">{nameFileCV ? nameFileCV : dataForm.ajouterUnCv}</label>
							<Paperclip />
							<input type="file" id="file-cv" name="file-cv" accept=".pdf" onChange={(e)=> getFileNameCV(e.target.value)} />
							<p className="form-message__error">{messageCV}</p>
						</div>
						<div className="group-input">
							{/* <label for="file-letter">{dataForm.ajouterUneLettreDeMotivation}</label> */}
							<label for="file-letter">{nameFileLetter ? nameFileLetter : dataForm.ajouterUneLettreDeMotivation}</label>
							<Paperclip />
							<input type="file" id="file-letter" name="file-letter" accept=".pdf" onChange={(e)=> getFileNameLetter(e.target.value)}/>
							<p className="form-message__error">{messageLetter}</p>
						</div>
