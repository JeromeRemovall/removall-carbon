...
> = ({
  ...
  useEffect(() => {
    ...
    setTagFilter(filtre);
    setMaxPage(
      Math.ceil(itemFilter.length / itemsPerPage)
    );
    filterItem();
  }, [items]);
  ...
  const filterItem = () => {
    ...
    if (tagFilter === "") {
      ...
      itemsFiltered = copieItems.filter(
        (item: any) =>
          item.tags?.nodes
    ...
    setMaxPage(
      Math.ceil(item.length / itemsPerPage)
    );
              ...
              {itemsToShow?.map(
                (event, index) => (
                  <Events
                    ...
                    isSpeaker={
                      event.events.speaker
                    }
                    lien={
                      event.events.lienCliquable
                    }
